...
const InputField: React.FC<InputFieldProps> = ({ type, name, placeholder, Icon, value, onChange }) => (
  <div className="relative w-full">
    ...
    <input
      ...
      value={value}
      onChange={onChange}
      className="w-full h-8 pl-10 py-3 px-4 bg-[#f0f0f0] rounded-[10px] placeholder-[#696969] focus:outline-none focus:ring-0"
    />
  </div>
);
...
export default function RegisterPage() {
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    Swal.fire({ icon: "success", title: "Register success", text: "Please login to continue" }).then(() => {
        router.push("/login"); // ✅ เปลี่ยนเส้นทางไปหน้าที่ต้องการ
    });
  };

  return (
    <div className="flex h-screen w-screen bg-cover bg-center bg-opacity-60" style={{ backgroundImage: "url('https://i.postimg.cc/qBcGFBv4/man2new.jpg')" }}>
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-center items-center p-10 bg-white rounded-l-[40px] shadow-3xl overflow-hidden shadow-lg">
        <div className="text-center mb-6">
          <img src="https://i.postimg.cc/LsBzbNmv/image-2025-03-16-034214396.png" alt="Logo" className="h-[120px] w-[120px] rounded-[35px] mx-auto mb-1 shadow-lg object-cover max-w-full max-h-full" />
        </div>
        ...
        <form onSubmit={handleSignUp} className="w-full max-w-sm space-y-7">
            ...
            // className="w-full py-3 px-4 bg-gray-200 text-gray-800 rounded-full placeholder-gray-500 focus:outline-none focus:ring-0"
            />
